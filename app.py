...
with tab3:
    ...
    if st.session_state.notion_token and st.session_state.notion_database_id:
        ...
        with st.expander("📚 Notionデータベースの必要プロパティ"):
            ...
            **注意**:
            - ISBNは「テキスト」タイプを推奨します。「数値」でも動作しますが先頭のゼロが削除されます。
            - Publishedは「日付」タイプ推奨。「テキスト」でも動作します。
            - Pagesは「数値」タイプ必須。「テキスト」でも動作します。
            - Coverプロパティはオプションです。作成すると表紙画像がプロパティとページカバーの両方に設定されます。

            **重要**: これらのプロパティが存在しない場合、対応するデータは登録されません。
